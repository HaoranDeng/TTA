...
| Paper `+ Act. Quant.` | 87.0 | 84.1 | 91.9 | 90.7 | 78.3 | 91.2 | 70.3 | 31.8 |
...
- July 21 post-training checkpoint probe: switching the FP16 baseline from `Qwen/Qwen3-1.7B-Base` to official `Qwen/Qwen3-1.7B` did not improve paper alignment in this evaluator. Best direct post-training prompt was `plain + instruction + 8-shot GLUE`, with GLUE `79.17`, MMLU-Pro `25.00`, SQuADv2 raw F1 `10.46`, and same-slice no-answer oracle F1 `60.24`. After paper-task adaptation from the post-training checkpoint, the best current row is `eval_taskadapt_posttrain_noanswer_continue_qwen3_1p7b_instruction_paperall_3000_lr1e6_g8_squad64`, with GLUE `80.47`, MMLU-Pro `26.56`, SQuADv2 raw F1 `14.32`, and same-slice oracle `59.58`. Its gaps to paper FP16 are `-8.33` GLUE, `-6.54` MMLU-Pro, and `-58.48` raw SQuAD F1, so this branch is not a better quantization starting point than the Base+task-adapted checkpoint.
- July 20 SmoothQuant-style activation scaling probe, milder scaling: `lutllm_smoothalpha03_taskadapt_squadrepeat4_4000_all196_ka512_km5_sample4096_chunk16m_centersonly_lr5e6_recon1_task1_steqat3000_squad32_ppl64` quantizes all 196 transformer-block linears with `Ka=512`, `subdim=2`, Chebyshev assignment, centers-only STE-QAT, and SmoothQuant-style input scaling `alpha=0.3`. It gets GLUE `82.29`, MMLU-Pro `21.88`, SQuADv2 raw F1 `38.54`, same-slice no-answer oracle F1 `63.54`, and WikiText PPL `50.38`. The gap to the paper `+ Act. Quant.` row is `-4.91` GLUE, `-9.93` MMLU-Pro, and `-31.76` raw SQuAD F1; the same-slice SQuAD oracle is still `-6.76` and is not paper-equivalent. This is better than `alpha=0.7` and modestly better than the comparable no-smooth centers-only run, but it is still not a reproduction.
- July 20 SmoothQuant-style activation scaling probe: `lutllm_smoothalpha07_taskadapt_squadrepeat4_4000_all196_ka512_km5_sample4096_chunk16m_centersonly_lr5e6_recon1_task1_steqat3000_squad32_ppl64` quantizes all 196 transformer-block linears with `Ka=512`, `subdim=2`, Chebyshev assignment, centers-only STE-QAT, and SmoothQuant-style input scaling `alpha=0.7`. It gets GLUE `80.21`, MMLU-Pro `12.50`, SQuADv2 raw F1 `34.38`, same-slice no-answer oracle F1 `60.42`, and WikiText PPL `75.77`. The gap to the paper `+ Act. Quant.` row is `-6.99` GLUE, `-19.30` MMLU-Pro, and `-35.93` raw SQuAD F1; the same-slice SQuAD oracle is still `-9.88` and is not paper-equivalent. The final QAT loss rose to `3.65` with reconstruction loss `2.82`, so strong `alpha=0.7` scaling is currently a negative result rather than a reproduction improvement.
- Hardware estimate for these SmoothQuant-style `Ka=512` all-196 act-only runs: `264,241,152` activation-center scalar values, `704,643,072` lookup-table reads per token, `132,120,576` centroid-distance vectors per token during activation encoding, `2,322,432` activation-code bits per token, and `688,128` MiB expanded FP16 activation-LUT storage. Smooth scales add `516,096` per-input-channel scale values; `alpha=0.3` ranges from about `0.58` to `96.06`, while `alpha=0.7` ranges from about `0.0034` to `914.76`.
...
- New SQuAD finding: most of the FP16 SQuAD gap comes from the SQuAD v2 no-answer decision protocol. On 64 validation examples, the SQuAD-repeat4 checkpoint has raw generative F1 `39.06`; a threshold learned on the next 64 validation examples gives held-out F1 `53.65`, while a same-slice diagnostic `No Answer` likelihood oracle reaches F1 `68.23`. The corresponding gaps to paper FP16 SQuAD are `-33.74`, `-19.15`, and `-4.57`.
- The same diagnostic now exists for an all-196 Act Quant rerun. `lutllm_taskadapt_noanswer_continue3000_all196_ka512_km5_sample4096_chunk16m_dense1e8_lr5e6_recon1_task1_oracle_save_steqat3000_squad64_ppl64` gets raw SQuAD F1 `33.28`; reloading the saved state and applying a held-out 64-sample `No Answer` threshold gives `54.11`; the same-slice oracle reaches `64.32`. The SQuAD gap to the paper `+ Act. Quant.` row is therefore `-37.02` raw, `-16.19` held-out calibrated, and `-5.98` oracle. This run is worse than the previous best on GLUE/MMLU (`80.47`/`17.19`), so it is a SQuAD-protocol diagnostic rather than a new best accuracy run.
...
| SQuAD-repeat4 checkpoint, held-out 64-sample `No Answer` threshold | 53.65 | -19.15 |
| SQuAD-repeat4 checkpoint, `No Answer` likelihood oracle threshold | 68.23 | -4.57 |

Interpretation: the SQuAD gap is not explained by generation length, a simple prompt variant, or simple postprocessing. The largest missing evaluation detail is the SQuAD v2 no-answer decision. In the formal 64-sample evaluator, 38/64 examples are unanswerable but raw generation emits only 4 empty predictions. A held-out threshold helps but is still much worse than the same-slice oracle, so the exact paper SQuAD protocol remains unresolved. The runner now supports `--squad-no-answer-oracle`, `--squad-no-answer-calibration-samples`, and `--eval-only-squad` for future FP16/all-196 quantized reruns.
...
The fixed-label task adaptation improves the FP16 GLUE/MMLU starting point, but SQuAD remains far below the paper FP16 baseline. Quantizing all 196 linears from that checkpoint now drops GLUE by about 1 point in the best task-adapted centers-only run. Init-only activation VQ is much worse, so the simplified QAT helps. The best centers-only branch uses strong KMeans initialization, lower center LR (`5e-6`), and the paper-like task/reconstruction ratio 1.0/0.1, reaching GLUE 83.59, still 3.61 points below the paper `+ Act. Quant.` row. Dense-weight QAT supports the inference that the paper's first stage trains both weights and activation centroids: it reaches GLUE 85.42 on a 32/task diagnostic, but a 64/task confirmation falls to 82.29, so the small-sample GLUE gain is not stable yet. Larger `Ka=512/1024` codebooks improve PPL and sometimes SQuAD/MMLU, but do not recover GLUE by themselves. A non-chunked `Ka=512` run from the improved checkpoint OOMed during SQuAD eval; chunked activation-distance evaluation avoids that peak memory problem and completed 32-sample diagnostics. `Ka=1024` cuts PPL to 33.69 but leaves GLUE at 81.77, so larger activation codebooks alone are insufficient. Increasing the QAT task pool to 2048 examples/task, raising dense LR to `3e-8` or `1e-7`, KMeans10, 8192 calibration, SQuAD `No Answer` label repair, SQuAD-weighted baseline adaptation, and inferred adjustable STE gradients all fail to close the gap. Strict `c_a=64`, L2 assignment, reconstruction-only training, smaller STE input gradients, `subdim=1`, higher LR, lower LR `3e-6`, and longer 4000-step training also fail. Full-layer `soft_hard` assignment with `Ka=256` and `Ka=64` OOMs on a 140GB H200 in this unfused PyTorch implementation.
...
| `lutllm_taskadapt_squadrepeat4_4000_all196_ka512_km5_sample4096_chunk16m_dense1e8_lr5e6_recon1_task1_steqat3000_squad32_ppl64` | SQuAD-repeat-4 4000-update checkpoint, `Ka=512`, dense-weight QAT LR `1e-8`, task 1.0 + recon 1.0, same-slice SQuAD oracle, 32/task | 81.25 | -5.95 vs Act | 21.88 | -9.93 vs Act | 40.62 raw / 57.29 oracle | -29.68 raw / -13.01 oracle vs Act | 72.91 |
| `lutllm_taskadapt_squadrepeat4_4000_all196_ca64_km5_sample4096_chunk16m_dense1e8_lr5e6_recon1_task1_steqat3000_squad32_ppl64` | same checkpoint, strict public `c_a=64`, dense-weight QAT LR `1e-8`, task 1.0 + recon 1.0, same-slice SQuAD oracle, 32/task | 71.35 | -15.85 vs Act | 9.38 | -22.43 vs Act | 34.97 raw / 68.30 oracle | -35.33 raw / -2.00 oracle vs Act | 127.65 |
| `lutllm_smoothalpha03_taskadapt_squadrepeat4_4000_all196_ka512_km5_sample4096_chunk16m_centersonly_lr5e6_recon1_task1_steqat3000_squad32_ppl64` | same SQuAD-repeat-4 checkpoint, `Ka=512`, centers-only QAT, SmoothQuant-style input scaling `alpha=0.3`, task 1.0 + recon 1.0, same-slice SQuAD oracle, 32/task | 82.29 | -4.91 vs Act | 21.88 | -9.93 vs Act | 38.54 raw / 63.54 oracle | -31.76 raw / -6.76 oracle vs Act | 50.38 |
| `lutllm_smoothalpha07_taskadapt_squadrepeat4_4000_all196_ka512_km5_sample4096_chunk16m_centersonly_lr5e6_recon1_task1_steqat3000_squad32_ppl64` | same SQuAD-repeat-4 checkpoint, `Ka=512`, centers-only QAT, SmoothQuant-style input scaling `alpha=0.7`, task 1.0 + recon 1.0, same-slice SQuAD oracle, 32/task | 80.21 | -6.99 vs Act | 12.50 | -19.30 vs Act | 34.38 raw / 60.42 oracle | -35.93 raw / -9.88 oracle vs Act | 75.77 |
...
| `eval_taskadapt_noanswer_continue_squadrepeat16_paperall_3000_lr5e7_g8_squad64` | FP16, continued No Answer checkpoint with SQuAD repeat 16, LR `5e-7` | 84.64 | -4.16 vs FP16 | 37.50 | +4.40 vs FP16 | 36.46 | -36.34 vs FP16 | - |
| `lutllm_taskadapt_noanswer_continue3000_all196_ka512_km5_sample4096_chunk16m_dense1e8_lr5e6_recon1_task1_steqat3000_squad32_ppl64` | 3000-update No Answer checkpoint, `Ka=512`, dense-weight QAT LR `1e-8`, task 1.0 + recon 1.0, 32/task | 85.42 | -1.78 vs Act | 18.75 | -13.05 vs Act | 38.54 | -31.76 vs Act | 56.23 |
| `lutllm_taskadapt_noanswer_continue3000_all196_ka512_km5_sample4096_chunk16m_dense1e8_lr5e6_recon1_task1_steqat3000_squad64_ppl64` | same as above, 64/task confirmation | 82.29 | -4.91 vs Act | 20.31 | -11.49 vs Act | 33.28 | -37.02 vs Act | 51.05 |
| `lutllm_taskadapt_noanswer_continue3000_all196_ka512_km5_sample4096_chunk16m_dense1e8_lr5e6_recon1_task1_oracle_save_steqat3000_squad64_ppl64` | same config rerun with saved Act Quant state and SQuAD no-answer oracle, 64/task | 80.47 | -6.73 vs Act | 17.19 | -14.61 vs Act | 33.28 raw / 64.32 oracle | -37.02 raw / -5.98 oracle vs Act | 52.19 |
| `lutllm_actquant_all196_ka512_dense1e8_oracle_calib64_squad64_loadedstate_fastinit` | same saved all-196 Act Quant state, SQuAD-only held-out no-answer calibration; fast init only builds matching modules before loading state | - | - | - | - | 33.28 raw / 54.11 calibrated / 64.32 oracle | -37.02 raw / -16.19 calibrated / -5.98 oracle vs Act | - |
| `lutllm_taskadapt_noanswer_continue3000_all196_ka512_km5_sample4096_chunk16m_dense1e8_lr5e6_recon1_task1_seed789_steqat3000_squad64_ppl64` | same best `Ka=512` dense-QAT branch, seed 789, 64/task | 81.51 | -5.69 vs Act | 21.88 | -9.93 vs Act | 37.50 | -32.80 vs Act | 48.30 |
| `lutllm_taskadapt_noanswer_continue3000_all196_ka512_km5_sample4096_chunk16m_fixedcenters_dense1e8_recon1_task1_steqat3000_squad64_ppl64` | same best branch but activation centers frozen, dense-weight QAT only, 64/task | 81.51 | -5.69 vs Act | 14.06 | -17.74 vs Act | 34.58 | -35.72 vs Act | 52.43 |
| `lutllm_taskadapt_noanswer_continue3000_all196_ka512_km5_sample4096_chunk16m_dense1e8_lr5e6_recon1_task1_affinecorr_steqat3000_squad64_ppl64` | same best branch with post-QAT affine output correction, 64/task | 80.99 | -6.21 vs Act | 21.88 | -9.92 vs Act | 33.47 | -36.83 vs Act | 54.51 |
| `lutllm_taskadapt_noanswer_continue3000_all196_ka512_km5_sample4096_chunk16m_dense1e8_lr5e6_recon1_task1_adjgrad_in05to1_center025to1_steqat3000_squad64_ppl64` | same best `Ka=512` dense-QAT branch, STE input gradient `0.5->1.0`, center gradient `0.25->1.0`, 64/task | 80.73 | -6.47 vs Act | 15.62 | -16.18 vs Act | 32.24 | -38.06 vs Act | 35.96 |
| `lutllm_taskadapt_noanswer_continue3000_all196_ka512_km5_sample4096_chunk16m_dense1e8_lr5e6_recon1_task1_ingrad0_steqat3000_squad64_ppl64` | same best `Ka=512` dense-QAT branch, STE input gradient disabled, 64/task | 80.47 | -6.73 vs Act | 18.75 | -13.05 vs Act | 32.24 | -38.06 vs Act | 66.12 |
| `lutllm_taskadapt_noanswer_continue3000_all196_ka512_km5_sample4096_chunk16m_dense1e8_lr5e6_recon1_task1_steqat1500_squad64_ppl64` | same best `Ka=512` dense-QAT branch, 1500 QAT steps, 64/task | 79.17 | -8.03 vs Act | 21.88 | -9.93 vs Act | 33.80 | -36.50 vs Act | 51.55 |
| `lutllm_taskadapt_noanswer_continue3000_all196_ka512_km5_sample4096_chunk16m_dense1e8_lr5e6_recon1_task1_l2_steqat3000_squad64_ppl64` | same best `Ka=512` dense-QAT branch, L2 activation assignment, 64/task | 79.17 | -8.03 vs Act | 17.19 | -14.61 vs Act | 33.28 | -37.02 vs Act | 36.28 |
| `lutllm_taskadapt_noanswer_continue3000_all196_ka512_km5_sample4096_chunk16m_fixedcenters_dense1e8_affinecorr_steqat3000_squad64_ppl64` | frozen activation centers plus affine output correction, 64/task | 79.43 | -7.77 vs Act | 20.31 | -11.49 vs Act | 33.02 | -37.28 vs Act | 54.43 |
| `lutllm_taskadapt_noanswer_continue_squadrepeat16_3000_all196_ka512_km5_sample4096_chunk16m_dense1e8_lr5e6_recon1_task1_steqat3000_squad64_ppl64` | SQuAD-repeat-16 checkpoint, same `Ka=512` dense-QAT branch, 64/task | 79.69 | -7.51 vs Act | 20.31 | -11.49 vs Act | 33.28 | -37.02 vs Act | 64.23 |
| `lutllm_taskadapt_noanswer_continue3000_all196_ka512_km5_sample4096_chunk16m_dense1e8_lr5e6_recon01_task1_steqat3000_squad32_ppl64` | same dense LR `1e-8`, task 1.0 + recon 0.1, 32/task | 80.21 | -6.99 vs Act | 18.75 | -13.05 vs Act | 41.67 | -28.63 vs Act | - |
| `lutllm_taskadapt_noanswer_continue3000_all196_ka512_km5_sample4096_chunk16m_dense1e8_wd0_lr5e6_recon1_task1_steqat3000_squad32_ppl64` | same dense LR `1e-8`, `weight_decay=0`, task 1.0 + recon 1.0, 32/task | 80.73 | -6.47 vs Act | 12.50 | -19.30 vs Act | 38.54 | -31.76 vs Act | 54.67 |
| `lutllm_taskadapt_noanswer_continue3000_all196_ka512_km5_sample4096_chunk16m_dense3e8_lr5e6_recon1_task1_steqat3000_squad32_ppl64` | same as above, dense-weight QAT LR `3e-8`, 32/task | 80.21 | -6.99 vs Act | 9.38 | -22.43 vs Act | 38.54 | -31.76 vs Act | 52.35 |
| `lutllm_taskadapt_noanswer_continue3000_all196_ca64_km5_sample4096_chunk16m_dense1e8_lr5e6_recon1_task1_steqat3000_squad64_ppl64` | strict public `c_a=64`, dense LR `1e-8`, task 1.0 + recon 1.0, 64/task | 75.26 | -11.94 vs Act | 9.38 | -22.43 vs Act | 29.53 | -40.77 vs Act | 134.20 |
| `lutllm_taskadapt_noanswer_continue3000_all196_ka512_km5_sample4096_chunk16m_dense1e8_lr5e6_recon1_task1_samples2048_steqat3000_squad64_ppl64` | same best `Ka=512` dense-QAT branch, but 2048 QAT task samples/task, 64/task | 77.08 | -10.12 vs Act | 14.06 | -17.74 vs Act | 35.27 | -35.03 vs Act | 38.34 |
...
| `lutllm_taskadapt_noanswer_continue3000_all196_ka1024_km5_sample4096_chunk16m_centersonly_lr5e6_recon1_task1_steqat3000_squad32_ppl64` | 3000-update No Answer checkpoint, `Ka=1024`, chunked distance eval, task 1.0 + recon 1.0, 32/task | 81.77 | -5.43 vs Act | 18.75 | -13.05 vs Act | 38.54 | -31.76 vs Act | 33.69 |
| `lutllm_taskadapt_noanswer_continue3000_all196_ka512_km5_sample4096_chunk16m_centersonly_lr5e6_recon1_task1_steqat3000_squad32_ppl64` | 3000-update No Answer checkpoint, `Ka=512`, chunked distance eval, task 1.0 + recon 1.0, 32/task | 81.77 | -5.43 vs Act | 18.75 | -13.05 vs Act | 38.54 | -31.76 vs Act | 54.00 |
| `lutllm_taskadapt_noanswer_continue3000_all196_ka512_km5_sample4096_chunk16m_centersonly_lr5e6_recon01_task1_steqat3000_squad32_ppl64` | 3000-update No Answer checkpoint, `Ka=512`, chunked distance eval, task 1.0 + recon 0.1, 32/task | 81.77 | -5.43 vs Act | 9.38 | -22.43 vs Act | 38.54 | -31.76 vs Act | 52.70 |
| `lutllm_taskadapt_noanswer_continue3000_all196_ka512_km5_sample4096_chunk16m_centersonly_lr5e6_recon1_task1_biascorr_steqat3000_squad32_ppl64` | same as above with post-QAT per-output bias correction, 32/task | 79.69 | -7.51 vs Act | 21.88 | -9.93 vs Act | 37.50 | -32.80 vs Act | 53.79 |
| `lutllm_taskadapt_noanswer_continue3000_all196_ka512_km5_sample4096_chunk16m_centersonly_lr1e5_recon1_task03_steqat2000_squad32_ppl64` | 3000-update No Answer checkpoint, `Ka=512`, chunked distance eval, task 0.3 + recon 1.0, 32/task | 79.69 | -7.51 vs Act | 25.00 | -6.80 vs Act | 38.54 | -31.76 vs Act | 51.60 |
...
July 13 continuation: moving the same all-196 dense-QAT recipe to the stronger SQuAD-repeat-4 4000-update checkpoint did not reproduce the paper. `Ka=512` lands at GLUE `81.25`, MMLU-Pro `21.88`, raw SQuAD `40.62`, and PPL `72.91`, still `-5.95`, `-9.93`, and `-29.68` below the paper `+ Act. Quant.` targets. The strict public `c_a=64` rerun is worse on GLUE/MMLU/PPL. Its SQuAD same-slice oracle reaches `68.30`, but this is a threshold oracle on the evaluation slice, not a paper-equivalent metric. The next implemented hypothesis is `--reconstruction-target original`, which fixes the reconstruction teacher to the original dense linear instead of the already-updated QAT weight; it was not launched on July 13 because scai7 had no H200 with enough free memory for another all-196 run.
...
| all196 `Ka=1024` | 196 | 528,482,304 | 1,376,256.0 MiB | 704,643,072 | 2,580,480 | 264,241,152 |
