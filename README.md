...
Current best first-step row is still not a reproduction: the best all-196 Act Quant result is `lutllm_taskadapt_fixedlabels1000_all196_ka256_km5_sample4096_centersonly_lr5e6_recon01_task1_steqat3000_squad64_ppl64`, with GLUE `83.59`, MMLU-Pro `12.50`, and SQuADv2 F1 `31.63`. That remains `-3.61`, `-19.30`, and `-38.67` below the paper `+ Act. Quant.` targets. The July 5-7 attempts below improved the FP16 baseline GLUE to `85.16` and SQuAD to `39.06`, but the all-196 activation-QAT step did not preserve that baseline; KMeans10, 8192 calibration, inferred adjustable STE gradients, SQuAD `No Answer` label repair, SQuAD-weighted task adaptation, and larger `Ka=512` did not close the GLUE gap. Chunked `Ka=512` 32-sample diagnostics raise SQuAD to `38.54`; `task=1.0/recon=1.0` keeps GLUE at `81.77` and improves MMLU to `18.75`, but it is still `-5.43`, `-13.05`, and `-31.76` below the paper target. Post-QAT per-output bias correction improves MMLU to `21.88` but drops GLUE to `79.69`, so it is not the missing reproduction detail.
