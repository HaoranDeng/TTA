...
The fixed-label task adaptation improves the FP16 GLUE/MMLU starting point, but SQuAD remains far below the paper FP16 baseline. Quantizing all 196 linears from that checkpoint still drops GLUE by about 7 points in the best task-adapted run. Stronger KMeans initialization, higher LR, longer training, and the task-heavy LR `1e-4` branch all fail to close the gap, so this route still does not reproduce the paper first-stage row.
