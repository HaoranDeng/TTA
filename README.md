...
- The same diagnostic now exists for an all-196 Act Quant rerun. `lutllm_taskadapt_noanswer_continue3000_all196_ka512_km5_sample4096_chunk16m_dense1e8_lr5e6_recon1_task1_oracle_save_steqat3000_squad64_ppl64` gets raw SQuAD F1 `33.28`, but the `No Answer` likelihood oracle reaches `64.32`, reducing the SQuAD gap to the paper `+ Act. Quant.` row from `-37.02` to `-5.98`. This run is worse than the previous best on GLUE/MMLU (`80.47`/`17.19`), so it is a SQuAD-protocol diagnostic rather than a new best accuracy run.
