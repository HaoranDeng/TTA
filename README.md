...
- Hardware estimate for that SmoothQuant-style `Ka=512` all-196 act-only run: `264,241,152` activation-center scalar values, `704,643,072` lookup-table reads per token, `132,120,576` centroid-distance vectors per token during activation encoding, `2,322,432` activation-code bits per token, and `688,128` MiB expanded FP16 activation-LUT storage. Smooth scales add `516,096` per-input-channel scale values, ranging from about `0.0034` to `914.76`.
