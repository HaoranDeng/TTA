...
Interpretation: fixed-label task adaptation improves the FP16 GLUE/MMLU starting point and reduces the baseline-alignment problem for those two metrics, but it still leaves SQuAD far below the paper FP16 row. Quantizing all 196 target linears from that checkpoint still drops GLUE by about 7 points in the best task-adapted run. Stronger KMeans initialization, L2 assignment, reconstruction-only training, smaller STE input gradients, higher LR, longer training, and the task-heavy LR `1e-4` branch all fail to close the gap, so this branch also has not reproduced the paper's first `+ Act. Quant.` row. Full-layer `soft_hard` assignment with `Ka=256` and `Ka=64` OOMs on a 140GB H200 in this unfused PyTorch implementation.
