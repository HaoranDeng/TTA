...
- `--reconstruction-target original` makes that reconstruction term use the original frozen dense linear as teacher instead of the moving QAT dense weight. This is an inferred follow-up for the paper's missing trained-LUT/reconstruction detail.
...
- `--squad-no-answer-calibration-samples` learns a `No Answer` threshold on a separate SQuAD validation slice and applies it to the eval slice, so the no-answer diagnostic is no longer only same-slice oracle tuning.
- `--eval-only-squad` reruns SQuAD protocol diagnostics without repeating GLUE/MMLU-Pro.
- `--save-act-quant-state` and `--load-act-quant-state` allow large Act Quant checkpoints to be kept outside git, so future evaluator changes can be rerun without repeating the full all-196 QAT training pass.
...
Interpretation: the SQuAD gap is not explained by a short generation length, a simple prompt variant, or simple postprocessing. The largest missing evaluation detail is the SQuAD v2 no-answer decision. In the formal 64-sample evaluator, 38/64 examples are unanswerable but raw generation emits only 4 empty predictions. A held-out threshold learned on rows 64-127 helps, but it is still much worse than the same-slice oracle on rows 0-63, so the exact paper SQuAD protocol remains unresolved.
...
Interpretation: fixed-label task adaptation improves the FP16 GLUE/MMLU starting point and reduces the baseline-alignment problem for those two metrics, but it still leaves SQuAD far below the paper FP16 row. Quantizing all 196 target linears from that checkpoint now drops GLUE by about 1 point in the best task-adapted centers-only run. Init-only activation VQ is much worse, so the simplified QAT helps. The best centers-only branch uses strong KMeans initialization, lower center LR (`5e-6`), and the paper-like task/reconstruction ratio 1.0/0.1, reaching GLUE 83.59, still 3.61 points below the paper `+ Act. Quant.` row. July 5-8 follow-up runs show that dense-weight QAT is an important missing detail: `Ka=512` with dense LR `1e-8` reaches GLUE `85.42`, only `-1.78` below paper Act GLUE on 32/task, but the 64/task confirmation falls to GLUE `82.29`; a same-config rerun with saved Act Quant state reaches GLUE `80.47`, MMLU-Pro `17.19`, raw SQuAD F1 `33.28`, held-out calibrated SQuAD F1 `54.11`, and same-slice SQuAD oracle F1 `64.32`. That confirms the quantized SQuAD gap is largely a no-answer decision issue, but even held-out calibration is still `-16.19` below paper Act SQuAD and the run is not a new best for GLUE/MMLU. A seed-789 rerun reaches GLUE `81.51`, MMLU-Pro `21.88`, and SQuAD F1 `37.50`, so seed sensitivity changes the tradeoff but does not close the GLUE gap. Shortening QAT to 1500 steps gives GLUE `79.17`, so simple early stopping is not the missing detail. Increasing dense LR to `3e-8` drops GLUE to `80.21`, lowering the dense-QAT reconstruction ratio to `0.1` also drops GLUE to `80.21`, setting `weight_decay=0` gives GLUE `80.73` with worse MMLU, and increasing QAT task samples to 2048 drops 64/task GLUE to `77.08` despite improving PPL to `38.34`. The strict public `c_a=64` configuration from the improved checkpoint reaches only GLUE `75.26`, MMLU-Pro `9.38`, SQuAD F1 `29.53`, and PPL `134.20`; its reconstruction loss is visibly higher than the `Ka=512` branch, so the current unfused PyTorch reproduction cannot make the public codebook size match the paper. Larger `Ka=512/1024` codebooks improve PPL and sometimes SQuAD/MMLU but do not recover GLUE by themselves; a non-chunked `Ka=512` run from the improved checkpoint OOMed during long-context SQuAD eval. Chunked `Ka=512` 32-sample diagnostics from the improved checkpoint reached SQuAD `38.54`; changing from task 0.3 + recon 1.0 to task 1.0 + recon 0.1 improved GLUE from `79.69` to `81.77`, but MMLU fell from `25.00` to `9.38`. Keeping reconstruction strong at task 1.0 + recon 1.0 held GLUE at `81.77` and recovered MMLU to `18.75`, still below the paper targets. Increasing to `Ka=1024` keeps the same GLUE/MMLU/SQuAD row and lowers PPL to `33.69`, so the remaining gap is not just activation codebook capacity. Post-QAT per-output bias correction recovered MMLU to `21.88` but dropped GLUE to `79.69` and SQuAD to `37.50`. July 8-12 checks on the best `Ka=512` dense-QAT branch show that input-gradient-free STE, scheduled input/center STE gradients, L2 assignment, frozen activation centers, affine output correction, and SQuAD-repeat-16 continued training all remain below the 64/task confirmation. Reconstruction-only training, smaller STE input gradients, `subdim=1`, higher LR, lower LR `3e-6`, and longer 4000-step training also fail. Full-layer `soft_hard` assignment with `Ka=256` and `Ka=64` OOMs on a 140GB H200 in this unfused PyTorch implementation.
...
Hardware note: strict public `c_a=64` all-196 first-stage activation VQ stores 33,030,144 activation-center values and implies 86,016 MiB of expanded FP16 Act-LUT equivalent, with 704,643,072 lookups/token, 1,548,288 activation-code bits/token, and 16,515,072 centroid-distance vectors/token. For `Ka=256`, the same all-196 scope stores 132,120,576 activation-center values and implies 180,388,626,432 expanded FP16 Act-LUT entries (344,064 MiB), with the same 704,643,072 lookups/token. `Ka=512` doubles centers and expanded LUT entries (688,128 MiB FP16 equivalent) while lookups/token stay unchanged. `Ka=1024` doubles them again to 528,482,304 activation-center values and 1,376,256 MiB expanded FP16 Act-LUT equivalent; lookups/token are still 704,643,072, but centroid-distance vectors/token rise to 264,241,152 and act code bits/token rise to 2,580,480. The chunked activation-distance path limits the temporary `[tokens, M, Ka]` distance tensor and lets large-codebook SQuAD/MMLU eval finish; it is slower, but closer to the kind of fused/chunked implementation the paper likely used.
