...
| `lutllm_taskadapt_fixedlabels1000_all196_ka256_centersonly_lr1e5_recon1_task03_steqat1000_squad64_ppl64` | `Ka=256`, centers-only, task 0.3 + recon 1.0, 1000 steps | 64.58 | -22.62 vs Act | 10.94 | -20.86 vs Act | 17.80 | -52.50 vs Act | 261.02 |
...
| `lutllm_taskadapt_fixedlabels1000_all196_ka256_centersonly_lr1e5_recon1_task03_steqat500_squad64_ppl64` | `Ka=256`, centers-only, task 0.3 + recon 1.0, 500 steps | 56.77 | -30.43 vs Act | 6.25 | -25.55 vs Act | 21.56 | -48.74 vs Act | 187.80 |
...
Interpretation: fixed-label task adaptation improves the FP16 GLUE/MMLU starting point and reduces the baseline-alignment problem for those two metrics, but it still leaves SQuAD far below the paper FP16 row. Quantizing all 196 target linears from that checkpoint still drops GLUE by about 7 points in the best task-adapted run. Init-only activation VQ is much worse, so the simplified QAT helps, but stronger KMeans initialization, L2 assignment, reconstruction-only training, smaller STE input gradients, dense-weight QAT, `subdim=1`, higher LR, longer training, and the task-heavy LR `1e-4` branch all fail to close the gap. This branch also has not reproduced the paper's first `+ Act. Quant.` row. Full-layer `soft_hard` assignment with `Ka=256` and `Ka=64` OOMs on a 140GB H200 in this unfused PyTorch implementation.
